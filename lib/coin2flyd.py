...
class Coin2flyDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some coin2flyd methods
    ...
    @classmethod
    def from_coin2fly_conf(self, coin2fly_dot_conf):
        from coin2fly_config import Coin2flyConfig
        config_text = Coin2flyConfig.slurp_config_file(coin2fly_dot_conf)
        creds = Coin2flyConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = coin2flylib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = coin2flylib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = coin2flylib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return False # (getinfo['protocolversion'] >= config.min_coin2flyd_proto_version_with_sentinel_ping)
