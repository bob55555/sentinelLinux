...
class Coin2flyConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read coin2fly.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is coin2fly.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from coin2fly.conf
        ...
        # use default port for network if not specified in coin2fly.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from coin2fly.conf
