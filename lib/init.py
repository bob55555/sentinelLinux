...
def has_coin2fly_conf():
    import config
    import io

    valid_coin2fly_conf = False

    # ensure coin2fly_conf exists & readable
    #
    # if not, print a message stating that Coin2fly Core must be installed and
    # configured, including JSONRPC access in coin2fly.conf
    try:
        f = io.open(config.coin2fly_conf)
        valid_coin2fly_conf = True
    except IOError as e:
        print(e)

    return valid_coin2fly_conf
...
def main():
    ...
    if not has_coin2fly_conf():
        print("Coin2flyCore must be installed and configured, including JSONRPC access in coin2fly.conf")
        sys.exit(1)
