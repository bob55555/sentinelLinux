...
class GovernanceClass(object):
    ...
    def vote(self, coin2flyd, signal, outcome):
        return self.go.vote(coin2flyd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, coin2flyd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(coin2flyd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(coin2flyd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to coin2flyd "gobject list" output
    ...
    def coin2flyd_serialise(self):
        import coin2flylib
        return coin2flylib.SHIM_serialise_for_coin2flyd(self.serialise())
