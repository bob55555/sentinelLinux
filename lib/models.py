...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, coin2flyd):
        golist = coin2flyd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_coin2flyd(coin2flyd, item)
    ...
    @classmethod
    def import_gobject_from_coin2flyd(self, coin2flyd, rec):
        import decimal
        import coin2flylib
        import inflection
        ...
        # shim/coin2flyd conversion
        object_hex = coin2flylib.SHIM_deserialise_from_coin2flyd(object_hex)
        objects = coin2flylib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from coin2flyd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from coin2flyd, with every run
        ...
        # get/create, then sync payment amounts, etc. from coin2flyd - Coin2flyd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(coin2flyd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from coin2flyd! %s" % e)
            govobj.vote_delete(coin2flyd)
            return (govobj, None)
    ...
    def vote_delete(self, coin2flyd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(coin2flyd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, coin2flyd, signal, outcome):
        import coin2flylib
        ...
        output = coin2flyd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = coin2flylib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(coin2flyd, signal)

    def sync_network_vote(self, coin2flyd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = coin2flyd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 coin2fly addr, non-multisig
            if not coin2flylib.is_valid_coin2fly_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Coin2fly address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (coin2flylib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/Coin2flyDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import coin2flylib
        obj_data = coin2flylib.SHIM_serialise_for_coin2flyd(self.serialise())
    ...
    def prepare(self, coin2flyd):
        try:
            object_hash = coin2flyd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not coin2flylib.is_valid_coin2fly_address(addr, config.network):
    ...
    def hash(self):
        import coin2flylib
        return coin2flylib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, coin2flyd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - coin2flyd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, coin2flyd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - coin2flyd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, coin2flyd):
        now = int(time.time())
        return (self.created_at < (now - coin2flyd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, coin2flyd):
        if self.is_expired(coin2flyd):
            return False

        return True

    def is_deletable(self, coin2flyd):
        if self.is_expired(coin2flyd):
            return True
