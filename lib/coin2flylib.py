...
def is_valid_coin2fly_address(address, network='mainnet'):
    ...
    coin2fly_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Coin2fly address
        return False

    if (address_version != coin2fly_version):
        return False
...
# shims 'til we can fix the coin2flyd side
def SHIM_serialise_for_coin2flyd(sentinel_hex):
    ...
    # superblock => "trigger" in coin2flyd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # coin2flyd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    coin2flyd_hex = serialise(obj)
    return coin2flyd_hex


# shims 'til we can fix the coin2flyd side
def SHIM_deserialise_from_coin2flyd(coin2flyd_hex):
    from models import COIN2FLYD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(coin2flyd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('coin2fly.conf')
