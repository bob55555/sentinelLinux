...
def test_valid_coin2fly_address():
    from coin2flylib import is_valid_coin2fly_address

    main = valid_coin2fly_address()
    test = valid_coin2fly_address('testnet')

    assert is_valid_coin2fly_address(main) is True
    assert is_valid_coin2fly_address(main, 'mainnet') is True
    assert is_valid_coin2fly_address(main, 'testnet') is False

    assert is_valid_coin2fly_address(test) is False
    assert is_valid_coin2fly_address(test, 'mainnet') is False
    assert is_valid_coin2fly_address(test, 'testnet') is True


def test_invalid_coin2fly_address():
    from coin2flylib import is_valid_coin2fly_address

    main = invalid_coin2fly_address()
    test = invalid_coin2fly_address('testnet')

    assert is_valid_coin2fly_address(main) is False
    assert is_valid_coin2fly_address(main, 'mainnet') is False
    assert is_valid_coin2fly_address(main, 'testnet') is False

    assert is_valid_coin2fly_address(test) is False
    assert is_valid_coin2fly_address(test, 'mainnet') is False
    assert is_valid_coin2fly_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from coin2flylib import elect_mn
...
def test_parse_masternode_status_vin():
    from coin2flylib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % coin2flylib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(coin2flylib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(coin2flylib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(coin2flylib.blocks_to_seconds(16616)) == 2612035
